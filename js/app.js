...
const game = {
		...
		fight() {
			//we have an alive alien, that's done by initialize alien
		}
		initializeAlien {
			currentAlien.isDead() {
				if (prompt().option && pileOfAliens < 6) {
					//Assume that option has come first, so we know whether we are fight or flight.
					//generate alien in here
					generateNewAlien();
					return fight();
				} else(pileOfAliens() == true) {
					return gameOverMan();
				}
			}
			pileOfAliens() {
				if (deadAliens === 6) {
					console.log('It looks like they are all dead... way to go... murderer... how many orphans do you think you just... nevermind...')
				}
			}
			prompt() {
